    ...
    async function loadPosts() {
      try {
        ...
        posts.forEach(post => {
          ...
          confirmBtn.onclick = () => {
            const token = localStorage.getItem("token");
            const ime = localStorage.getItem("ime") + " " + localStorage.getItem("prezime");

            fetch(`https://selfmarking-backend.onrender.com/api/posts/validators/${post._id}`)
              .then(res => res.json())
              .then(data => {
                const većPotvrdio = data.confirmators.includes(ime);
                const metod = većPotvrdio ? "remove" : "add";

                fetch(`https://selfmarking-backend.onrender.com/api/posts/confirm/${post._id}?mode=${metod}`, {
                  method: "POST",
                  headers: {
                    "Authorization": "Bearer " + token
                  }
          ...
          denyBtn.onclick = () => {
            ...
            fetch(`https://selfmarking-backend.onrender.com/api/posts/validators/${post._id}`)
              .then(res => res.json())
              .then(data => {
                const većDemantovao = data.deniers.includes(ime);
                const metod = većDemantovao ? "remove" : "add";

                fetch(`https://selfmarking-backend.onrender.com/api/posts/deny/${post._id}?mode=${metod}`, {
                  method: "POST",
                  headers: {
                    "Authorization": "Bearer " + token
                  }
